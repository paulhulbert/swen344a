...
import {
  CHART_DAY_FIELDS, CHART_DATE_FORMATS, API_DATE_FORMAT, CHART_TYPES,
...
function processIndividualStockData(stock) {
  return stock.get('quote').filter((_, key) => STOCK_FIELDS_TO_KEEP.includes(key));
}
...
function fetchCachedChartData(todayDate, ticker, callback) {
  return firebase.database().ref(getStockChartPath(ticker, todayDate)).once('value').then((response) => {
    ...
    return callback(ticker, chartDataEntry.get('chartData'));
  });
}
